...
(function () {
  ...
  /* 「2-3,5-6,9-18([周])[01-02节]」这种教务系统标准写法。
     ...
     实测教务系统课表页面给的是「2-3,5-6,9-18(周)[03-04节]」（括号里没有中括号），
     所以中括号做成可选，两种都要认。 */
  var WP_RE = /^([\d,\-\s]+)\s*[（(]\s*\[?\s*([^\])]*?)\s*\]?\s*[)）]\s*\[?\s*([\d\-]+)\s*节\s*\]?$/;

  /* 宽松版：一行里同时出现周次和节次，顺序不限 */
  var WEEK_TOKEN = /(\d{1,2}(?:\s*-\s*\d{1,2})?(?:\s*,\s*\d{1,2}(?:\s*-\s*\d{1,2})?)*)\s*[（(]?\s*周/;
  var PERIOD_TOKEN = /\[?\s*(\d{1,2}(?:\s*-\s*\d{1,2})*)\s*节\s*\]?/;

  /* 只有周次的一行，例如「1-16周」「1,3,5周」「第1-8周」「9-18(周)」 */
  var WEEK_ONLY_RE = /^第?\s*[\d,\-\s]+\s*[（(]?\s*周\s*[)）]?$/;
   ...
   * 这样既能认下「1-16周(单)」，又不会把「认知实习 1-18周」或「C-5-103」误判成周次。
   * 教务系统页面那种「2-3,5-6,9-18(周)[03-04节]」里带节次中括号的也要算进去。
   */
  function isWeekLine(line) {
    ...
    return s.replace(/[\d,，、\-~至周单双奇偶()（）第\[\]节小]/g, '') === '';
  }
  ...
  function parseBlock(lines, day, fallbackCodes) {
    ...
    // 教务系统一个格子里塞多门课时，会用「---------------------」这种横线分隔，
    // 别把它带进课程名里（否则会出现「------ 形势与政策1」这种名字）
    var DASH_ONLY = /^[\-\s—–_=·]+$/;
    nameLines = nameLines.filter(function (l) { return l && !DASH_ONLY.test(l); });
    if (teacher && DASH_ONLY.test(teacher)) teacher = '';
    ...
    // ⑦ 既没有周次也没有节次线索，又带着分号，多半是「认知实习 1-18周;行业认知 1-18周」
    //    这类附注，不要当成课程收进来
  ...
  function fromWeekdayGrid(rows, headerRow, periods) {
    ...
    // 有的话，非节次行（比如最后那行「认知实习 1-18周」的附注）就只当备注，
